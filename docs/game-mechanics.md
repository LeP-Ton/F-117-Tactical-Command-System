...
地图固定为 `1000×1000 u`，网格间隔 `100 u`；所有玩家可见地图坐标以左下角为 `(0,0)`，X 轴向右、Y 轴向上。F-117 插入点固定为 `(100,100)`，因此显示在地图左下。每个任务使用独立的 `<节点 Seed>:EXTRACTION` 随机流，从中心点 `(100,900)`、`(900,900)`、`(900,100)` 中等概率选择一个 `100×100 u` 撤离区，因此三个候选分别显示在左上、右上和右下。目标中心的 X、Y 坐标均生成于 `300–700`，攻击半径为 `50 u`；所有初始、最终战增援及火控重部署雷达的中心坐标均限制在双轴 `200–800`。雷达无需对插入点或撤离区净空，真实覆盖可以进入这些区域。Canvas 内部仍使用左上原点，但生成输入、航点列表、飞行遥测、复盘坐标与天气预报均在边界转换为左下原点语义。任务最终准备时还会保证至少一部 Fire Control 完整覆盖目标攻击区并保留 `20 u` 余量。
