...
Recon 和 ELINT 任务成功后会永久提高本次 Run 后续任务的情报精度。因此，它们的价值是让后续航线规划依据更可靠，而不是直接削弱雷达。
...
- 飞机探测修正：保留的系统扩展参数，当前正式奖励池不会提供强化。
...
- `RECON`：成功获得 2 Intel，后续情报精度增加 6%。
- `ELINT`：成功获得 3 Intel，后续情报精度增加 10%。
- `STRIKE`：成功获得 1 Intel。
- `SEAD`：成功后 Enemy Alert 降低 8，并使后续雷达覆盖乘以 85%，最低为初始覆盖的 55%。
- `COMMAND_STRIKE`：成功后后续指挥链效率乘以 75%，最低为 45%。
- `DEEP_STRIKE`：当前主要承担高风险路线节点，没有额外专属持久奖励。
- `FINAL_STRIKE`：成功后赢得本次 Run。

Recon 与 ELINT 的累计情报精度加成上限为 24%，单次任务最终情报精度最高为 99%。

成功完成普通任务通常使 Enemy Alert 增加 2；失败会增加 10。后续任务的雷达覆盖还会乘以 `1 + Enemy Alert / 250`，所以连续高警戒会抵消一部分 SEAD 成果。

## 10. 当前奖励与 Build 状态

当前 Tactical Reward Pool 为空：

- 成功任务后直接返回 Campaign Map。
- 不出现 Roguelike 三选一奖励。
- Player Build、奖励注册和部分模块参数只作为未来扩展接口保留。
- 当前正式玩法差异来自程序生成地图、雷达、天气、Doctrine 和 Campaign 防空变化。

## 11. Enemy Adaptation

任务结束后，敌方只分析已经发生的历史，不读取下一任务尚未执行的航线：

- 地形利用：已飞航点落在地形遮蔽区内的比例。
- 南北航路偏好：已飞航点在地图纵向的位置分布。
- 直达倾向：起终点直线距离与实际已飞路线长度的比例。
- 接触容忍：任务中实际 Radar Contact 的数量。

有效任务样本会提高 `ADAPT LV`，目前最高为 5。后续任务会按画像确定性调整雷达部署：
...
- 玩家可用的 Tactical Reward 和 Build 奖励池。
- 正式难度设置、教程关卡和存档系统。
