...
每个 Run 包含三个顺序二选一阶段与一个 Final Strike。只有摧毁目标并成功进入撤离区，当前节点才变为 `COMPLETED`、同层另一节点变为 `EXPIRED`，随后解锁下一阶段。普通失败不会推进 Campaign，当前层保持可重试或改选。相同 Seed 会复现节点、地图、雷达网络、环境修正区、目标和初始情报精度。
