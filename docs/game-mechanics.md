...
Contact 的可信度、信号强度和指挥链效率共同决定跟踪增长速度。没有新 Contact 时，跟踪质量每秒下降 14；因此转向、离开覆盖、进入地形或天气都可能通过阻止新证据实现脱锁。
...
Commander 每秒评估一次全局 Awareness、Belief 和雷达状态，然后选择一个意图：
...
指挥链效率控制 Commander 对 Radar Operator 的命令偏置强度：

- 100%：完整施加协同命令。
- 较低数值：雷达更多依赖自己的 Contact 和本地判断，协同行为减弱。
