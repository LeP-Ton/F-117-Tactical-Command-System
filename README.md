...
右侧 `MAP ELEMENTS` 会说明当前地图中的飞机、目标、撤离区、航点、地形、天气和已知雷达；点击条目会在地图上高亮对应目标。任务与战役进度每秒自动保存到浏览器本地，刷新页面后会恢复；飞行中的存档将以暂停状态打开，避免离线期间推进任务。

F-117 满油拥有 `2000 u` 可用航程，等于当前地图两条边的总长度。燃油按真实飞行距离消耗，弯折航线同样计入；燃油耗尽时飞机停止并令当前任务失败。
...
雷达扫描线经过飞机时会按距离、朝向、地形遮蔽与动态天气计算探测概率。Cloud、Rain、Storm 与 Fog 会移动、改变范围和按周期演变类型，并分别令飞机减速 10%、20%、30% 与 15%；多个天气重叠时取最强减速。规划或暂停时可参考地图预测轮廓与 `WEATHER FORECAST` 的 `T+30/60/90s` 有误差预报。默认视图中的黄色区域表示玩家对敌方雷达的有限情报；敌方获得的 Radar Contact 只在 AI DEBUG 中显示。

防空网络包含三种真正不同的雷达：Early Warning 负责远程宽波束预警，Acquisition 负责中程搜索与目标获取，Fire Control 负责近程高精度跟踪和导弹制导。三者的覆盖、扫描周期、波束宽度、探测率、Contact 误差和锁定贡献均不同。每台雷达还会根据 Contact 证据在广域搜索、扇区搜索和聚焦跟踪间切换；AI DEBUG 会显示类型、当前模式与 W/S/F 效用评分。

每场任务至少保留一部 Fire Control 完整覆盖目标攻击区，并预留 `20 u` 覆盖余量。该约束在 SEAD、Enemy Alert、Enemy Adaptation 与 Final Strike 修正完成后重新校验，因此攻击目标始终需要面对目标区火控威胁，但进入范围仍不等于必然被探测或锁定。
