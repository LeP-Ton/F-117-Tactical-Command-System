...
`workflow/20260912215947-refactor-mission-areas-and-deployment-ranges.md` - 会话-17：集中重构飞机起点、目标区、雷达部署范围与三候选随机撤离区，删除撤离区雷达净空，保持目标区火控覆盖余量，并将存档载荷升级为兼容 v1 的 v2；其中坐标原点方向已由会话-18纠正，核对最终坐标语义时必须继续读取会话-18文档。
...
`workflow/20260903211626-responsive-viewport-and-fullscreen-icon.md` - 会话-2：重绘全屏四角图标，并以 1280×720 逻辑视口为基准对全部页面、文字、Canvas 与浮层实施 0.5×–2× 等比自适应，补偿地图坐标、像素密度和引导高亮；排查分辨率缩放、Canvas 点击偏移或引导错位时读取。
`workflow/20260903210152-add-fullscreen-display.md` - 会话-1：在顶部控制区增加浏览器全屏切换，支持进入、按钮再次退出、Escape/系统退出后的状态同步、中英文文案与不支持时的禁用提示；排查全屏入口、Fullscreen API 或顶部栏布局时读取。
...
- 全部页面、文字、Canvas 与浮层以 `1500×720` 最小逻辑视口为基准按实际视口等比缩放，最大为 `2×` 且极小视口继续缩小；1500px 宽度保证双语顶部栏单行，720px 高度约束非侧栏元素不超过视口，地图交互和任务引导坐标已补偿外层缩放。
...
- Mission 的静态地形、动态天气初始参数与演化、天气预报、雷达、目标和撤离区均由独立命名 Seed 确定生成；有限雷达情报按固定规则和逐雷达子 Seed 生成，相同任务时间可复现相同真实天气。
- 玩家地图坐标以左下角为 `(0,0)`、Y 轴向上，Canvas 内部左上原点只作为实现细节并在边界转换；飞机起点 `(100,100)` 显示在左下，撤离区中心 `(100,900)`、`(900,900)`、`(900,100)` 分别显示在左上、右上、右下。目标中心双轴 `300–700`、攻击半径 `50 u`，雷达中心双轴 `200–800`，且雷达不再执行起点或撤离区净空。
...
- Run、任务网络、Mission、成功复盘与当前工作区视图保存在浏览器本地；载荷 v2 使用 `BOTTOM_LEFT` 坐标标记并兼容读取 v1，v1 与无标记 v2 的规划任务重建，运行/结果/复盘保留旧地图；运行中的存档刷新后继续执行，旧版 `PAUSED` 存档迁移为 `RUNNING`。
- 右侧 `MAP ELEMENTS` 可说明并高亮地图元素，普通模式下雷达定位仍严格使用有限情报。
- Mission Generator 没有严格的路径可达性或数学可通关证明，只保证部署坐标边界和目标区最低 Fire Control 覆盖等局部约束。
