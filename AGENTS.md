...
- `RunState` 与 `MissionSession` 严格分离；Seed、事件系统、Campaign、Build 和 Enemy Adaptation 均保留独立扩展边界。
...
- Tactical Reward 与 Player Build 框架保留为扩展点，但当前正式奖励池为空，成功任务不展示三选一，也不向 Player Build 添加模块。
- 当前核心玩法不得依赖奖励模块；差异性应优先来自动态航线、程序生成雷达/地形/天气、敌方 Belief 与 Commander 行为。
...
- Enemy Adaptation 只分析已完成航点与历史 Contact，形成地形利用、南北航路、直达倾向和接触容忍画像，并据此调整后续雷达部署；禁止读取未来计划航点。
