...
- 玩家地图坐标统一以左下角为 `(0,0)`，X 轴向右、Y 轴向上；Canvas 与内部任务状态保留左上原点，并在生成输入和显示边界集中转换。飞机起点固定为玩家坐标 `(100,100)`；目标中心双轴范围为 `300–700`、攻击半径 `50 u`；雷达中心双轴范围为 `200–800`。每个任务通过独立 Mission Seed 子流从玩家坐标 `(100,900)`、`(900,900)`、`(900,100)` 三个中心等概率选择 `100×100` 撤离区，对应画面左上、右上和右下；雷达无需对起点或撤离区净空，探测范围允许覆盖两者。
...
- Run、Campaign、当前 Mission 与成功任务复盘每秒自动保存到浏览器 `localStorage`；当前载荷版本为 2 并兼容版本 1，新载荷带 `BOTTOM_LEFT` 地图坐标标记。v1 与会话-17期间无坐标标记的 v2 规划任务按节点 Seed 与既有任务收益重建并清除旧航线，运行中、结果与历史复盘保留原地图；带标记的 v2 原样恢复已保存的随机撤离区。刷新时飞行中的 Mission 保持执行并强制返回战术视图。
